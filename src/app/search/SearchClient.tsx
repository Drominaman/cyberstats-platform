...
import { Search, Filter, Calendar, Tag, Building2, Bookmark, Loader2 } from 'lucide-react'
...
export default function SearchPage() {
  ...
  const fetchResults = useCallback(async (reset: boolean) => {
    ...
    try {
      ...
      setHasMore(offsetRef.current < totalCount && newItems.length === LIMIT)
  ...
  }, [query, sortBy, loadingMore, hasMore])
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !loadingMore && !loading && stats.length > 0) {
          fetchResults(false)
        }
      },
  ...
  }, [hasMore, loadingMore, loading, stats.length, fetchResults])
                ...
                <div ref={loadMoreRef} className="py-8 text-center">
                  ...
                  {!hasMore && stats.length > 0 && (
                    <p className="text-gray-400">
                      Showing all {stats.length.toLocaleString()} results
                    </p>
                  )}
                </div>
              </>
